...
VideoCaptureAndroid::JavaVideoCapturer::JavaVideoCapturer(NativeRegistration* native_reg, std::unique_ptr<GlobalRef> audio_manager)
    : audio_manager_(std::move(audio_manager))
      ...
      , start_capture_(native_reg->GetMethodId("startCapture", "(II)Z")) 
{
...
bool VideoCaptureAndroid::JavaVideoCapturer::StartCapture(int width, int height) 
{
	return audio_manager_->CallBooleanMethod(start_capture_, width, height);
}
...
int32_t VideoCaptureAndroid::StartCapture(const VideoCaptureCapability& capability) 
{
	...
	if (!InitCaptureAndroidJNI() || !j_video_capturer_->StartCapture(capability.width, capability.height)) {
		UninitCaptureAndroidJNI();
		ALOGD("StartCapture%s, (%i x %i) Fail", GetThreadInfo().c_str(), capability.width, capability.height);
		return -1;
	}
